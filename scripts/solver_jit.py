...
@njit(cache=True)
def _run_core_ratio(state, neighbor_sum, is_conflicted, degree, offsets, nbrs,
                     A, B_arr, temps, candidate_draws, accept_draws, max_iters, target):
    n = state.shape[0]
    num_conflicted = 0
    for v in range(n):
        if is_conflicted[v]:
...
def petford_welsh_jit_ratio_schedule(graph, A=1.0, B=1.5, b=18, max_iters=1000, rng=None, target=None, init_fn=None):
    t0 = time.perf_counter()
    ...
    if callable(B):
        B_arr = np.asarray([B(it) for it in range(max_iters)], dtype=np.float64)
    elif np.isscalar(B):
        B_arr = np.full(max_iters, float(B))
    else:
    ...
    best_state, best_value, best_step = _run_core_ratio(
        state, neighbor_sum, is_conflicted, degree, offsets, nbrs,
        A, B_arr, temps, candidate_draws, accept_draws, max_iters, target_arg,
    )
    elapsed = time.perf_counter() - t0
