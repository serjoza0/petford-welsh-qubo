...
KNOWN_ALPHA = {
    ...
    "c-fat200-1": 12, "cpython-fat200-2": 24, "c-fat200-5": 58,
...
DEFAULT_MAX_ITERS = [5000, 20000, 100000, 500000, 2000000, 10000000, 50000000]
A = 1.0
B = 10.0
SEED = 0
SA_BETA = 0.5
...
def timed_init_fn(init_fn):
    """
    Wraps an init_fn so every call also records its own wall time, in call
    order, onto the returned wrapper's `.construction_times` list. Since
    run_multi_start calls init_fn exactly once per attempt in order
    seed, seed+1, ..., this list lines up 1:1 with result.attempts.
    """
    times = []

    def wrapped(graph, num_states, rng):
        t0 = perf_counter()
        state = init_fn(graph, num_states, rng)
        times.append(perf_counter() - t0)
        return state

    wrapped.construction_times = times
    return wrapped


def run_pw_sweep(graph, max_iters_list, num_attempts, seed, solver, init_fn):
    """
    (max_iters, best_value, elapsed) for every attempt at every max_iters
    level, with `elapsed` corrected to include init_fn's own construction
    time when the jit solver would otherwise have excluded it. Also returns
    every recorded construction_time (one per attempt per max_iters level,
    same value in expectation regardless of level since it only depends on
    the graph) so the caller can report/plot the standalone preprocessing
    cost of this init_fn.
    """
    points = []
    construction_times = []
    for max_iters in max_iters_list:
        wrapped = timed_init_fn(init_fn) if init_fn is not None else None
        result = run_multi_start(
            graph, solver=solver, A=A, B=B, b=4.0,
            max_iters=max_iters, num_attempts=num_attempts, seed=seed,
            init_fn=wrapped,
        )
        needs_fixup = (solver == "jit" and wrapped is not None)
        for i, a in enumerate(result.attempts):
            elapsed = a.elapsed + wrapped.construction_times[i] if needs_fixup else a.elapsed
            points.append((max_iters, a.best_value, elapsed))
        if wrapped is not None:
            construction_times.extend(wrapped.construction_times)
    return points, construction_times
...
def main():
    ...
    # Warm up numba's JIT compile for greedy_min_degree_init_jit here, once,
    # outside the timed loop -- otherwise the first measured "construction
    # time" below would silently include a one-off ~0.1-1s compile cost.
    greedy_min_degree_init_jit(graph, 1, np.random.default_rng(1))
    ...
    for init_label, init_fn in INIT_STRATEGIES.items():
        pw_points, ctimes = run_pw_sweep(graph, args.max_iters, args.num_attempts, SEED, args.solver, init_fn)
        all_points[init_label] = pw_points
        construction_times[init_label] = ctimes
    ...
    for init_label, ctimes in construction_times.items():
        if ctimes:
            print(f"\n{init_label} preprocessing: avg={np.mean(ctimes):.5f}s over {len(ctimes)} constructions")
    ...
    avg_construction = {
        label: float(np.mean(ctimes)) for label, ctimes in construction_times.items() if ctimes
    }
    for init_label, avg_ct in avg_construction.items():
        ax.axvline(avg_ct, color=COLOR_FOR[init_label], linestyle=":", linewidth=1.5, zorder=3)
    ...
    for init_label, avg_ct in avg_construction.items():
        handles.append(Line2D([0], [0], color=COLOR_FOR[init_label], linestyle=":", linewidth=1.5,
                               label=f"{init_label} preprocessing avg ({avg_ct:.4f}s)"))
    if sa_points:
    ...
    print(f"\nSaved plot to {out_path} ({n_points} PW points across {len(args.max_iters)} max_iters levels x "
          f"{len(INIT_STRATEGIES)} init strategies" + (f", {len(sa_points)} SA points" if sa_points else "") + ")")
